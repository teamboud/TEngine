...
public class Loader {
	...
	public int loadTexture(String fileName, float bias){
		...
		if(GLContext.getCapabilities().GL_EXT_texture_filter_anisotropic){
			...
			System.err.println("AnisotropicFiltering not supported");
	...
	public int loadTextureW(String fileName, float bias){
		Texture texture = null;
		try {
	...
	private int createVAO(){
		...
		vaos.add(vaoID); //Add it to the list
	...
	private void storeDataInAttributeList(int attributeNumber,int coordSize, float[] data){
		int vboID = GL15.glGenBuffers(); //Get the vbo id
		vbos.add(vboID); //Add it to the List
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vboID); //Bind it
		FloatBuffer buffer = storeDataInFloatBuffer(data); //Put the data
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW); //Store the buffer in the vbo
		GL20.glVertexAttribPointer(attributeNumber,coordSize,GL11.GL_FLOAT,false,0,0); //Store the vbo in the vao
